...
module.exports = {
      ...
      Invite.findOne({
        ...
        .then(invite => {
          if (invite) {
            ...
            res
              .status(400)
              .send({ message: 'Invitation code does not exist.' });
          }
        })
        .catch(err => {
          res
            .status(400)
            .send({ message: `Error getting invitation data: ${err}` });
        });
    }
  },
  ...
  requestInvitationCode: function(req, res) {
    ...
    if (!firstName || !lastName || !email) {
      res
        .status(400)
        .send({ message: 'firstName, lastName and email are mandatory.' });
        ...
        .then(invite => {
          if (invite) {
            ...
            Invite.create({
              ...
              res
                .status(200)
                .send({ message: 'Request invitation submitted.' });
            });
          }
        })
        .catch(err => {

...
module.exports = {
  ...
  generateArtist: () => {
    ...
    return {
      name: `${arrayHelpers.getRandomArrayItem(names)} ${arrayHelpers.getRandomArrayItem(surnames)}`,
      location: {
        city: arrayHelpers.getRandomArrayItem(cities)
      },
      id: uuid(),
      followers: Math.floor((Math.random() * 120239) + 8300),
      studio: Math.random() >= 0.5,
      avatar: arrayHelpers.getRandomArrayItem(photos),
      expertise: Math.random() >= 0.5 ? 'professional' : 'amateur',
      priceRange: Math.floor((Math.random() * 5) + 1),
      photos: [
        {
          thumbnailUrl: arrayHelpers.getRandomArrayItem(photos)
        },
        {
          thumbnailUrl: arrayHelpers.getRandomArrayItem(photos)
        },
